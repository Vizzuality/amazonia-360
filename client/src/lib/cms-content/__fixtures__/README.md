...
What `fetchTopics`, `fetchSubtopics` and `fetchIndicators` actually receive from the Payload
REST API, recorded from a database seeded with `pnpm seed:data`. They let `index.test.ts` pin
the CMS-to-app projection without CI needing Postgres, migrations and a seed.

`topics.*.json` and `subtopics.*.json` hold every record. `indicators.*.json` holds a sample
chosen to cover the branches of the boundary rather than the catalogue:
...
`imagery-tile` and `web-tile` have no rows in the catalogue, and nothing here covers them:
they are `Extract`ed off the generated types, so the compiler is what checks them.
